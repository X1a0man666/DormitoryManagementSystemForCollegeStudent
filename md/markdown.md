...
| 闭环归属（迭代五） | 学生发起购电 / 报修，由**宿管科**处理（售电、修理）；宿舍管理人员只做夜归 / 卫生 / 贵重登记，且登记限定其分管楼栋 | 与角色职责表一致：宿管科=楼栋分配 / 宿舍管理(人员)管理 / 修理 / 售电；宿舍管理(人员)=日常登记 |
| 楼栋性别显式化 | `Building` 增 `gender` 列；`genderText()` 优先显式值，按 A栋=男 / B栋=女 兜底推导 | 支持"真实楼名 + 性别"的楼栋分配；兼容既有命名约定数据 |
| 楼栋归属约束 | 有学生入住的楼栋不可改归属（须先腾空），空楼可划拨/解除；分配同步更新 `Building.collegeCode` 与 `College.buildingNames` | 避免学生档案与所在楼栋归属矛盾 |
...
- **学号规则（8 位）**：第 1-2 位=入学年份后两位；第 3-4 位=学院代码；第 5 位=专业代码（0-9，默认每学院 10 个专业）；第 6 位=班级代码（1-9，默认每专业 9 个班）；第 7-8 位=学号序号（01-99，默认每班 99 人）。**转专业后学号不变**。**课程设定（迭代三收紧，2026-09-09 扩班）**：每学院固定 **4 专业（代码 1-4）** × 每专业默认 **4 班** × 每班 **40 人**；软件学院专业 1 = 软件工程（专业年级代码 25201），**每届 9 班**（其余专业仍 4 班）。
- **宿舍楼（32 栋标准公寓，真实调研）**：27 本科 + 4 研究生 + 1 留学生；1-6 栋无独立卫浴、7-28 栋有独立卫浴；25 栋上床下桌；最高 7 层。**注意：迭代三改为课程设定模型**——每学院 2 栋（`<学院代码>A栋`=男、`<学院代码>B栋`=女，如 01A/20B），每层 45 间、每间 4 人，23-26 四届全量按学号顺序排宿，不再使用 1-32 栋命名。
...
| counselor26201 | 123456 | 辅导员 | 2026级软件学院·软件工程（专业年级代码 26201） |
